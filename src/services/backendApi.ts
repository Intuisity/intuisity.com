...
function syncProfileActivity(profile: unknown) {
  const typedProfile = profile as { authProvider?: string; email?: string; name?: string };
  const email = String(typedProfile?.email || "").trim().toLowerCase();
  if (!email || typedProfile.authProvider === "guest") return;

  try {
    const today = getDateKey();
    const storageKey = `intuisity-profile-activity-${email}-${today}`;
    ...
    syncModuleTime({
      ...
      startedAt: savedAt.toISOString(),
      clientChannel: getClientPlatformDetails().clientChannel,
      deviceCategory: getClientPlatformDetails().deviceCategory
    });
  } catch {
    // Profile activity is helpful for reports, but should never interrupt sign in.
  }
}
